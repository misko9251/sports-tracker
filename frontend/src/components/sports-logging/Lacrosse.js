...
function Lacrosse() {
    ...
    const endPeriod = () => {
        setGameStats([{event: `Period ${currentQuarter} has ended.`}, ...gameStats])
    ...
    const endGame = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:2121/stats/updateLacrosseStats/${teamId}/event/${eventId}`, {
                credentials: 'include',
                method: 'POST',
                ...
                <section style={{flexGrow: 1, backgroundColor: 'rgb(47, 47, 47)'}}>
                    ...
                    <div className='team-scoring-container'>
                        ...
                        <div className='next-period'>
                            {currentQuarter < 3 && <button onClick={endPeriod}>End Quarter</button>}
                            {currentQuarter >= 3 && <button onClick={endGame}>End Game</button>}
                        </div>
                    </div>
